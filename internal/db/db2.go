...
const logfile string = "logs-fulltxtsearch.db"
...
func init() {
	...
	const create string = `CREATE VIRTUAL TABLE IF NOT EXISTS logs USING fts5(timestamp, message);`
...
func (q *SelectMessageQuery) Build() string {
	...
	if q.pattern != "" {
		cond := fmt.Sprintf("%s MATCH '%s'", q.table, q.pattern)
		conditions = append(conditions, cond)
	}

	if q.startingId > 0 {
		cond := fmt.Sprintf("rowid > %d", q.startingId)
		conditions = append(conditions, cond)
	}

	query := fmt.Sprintf("SELECT rowid,timestamp,message FROM %s", q.table)
	if 0 < len(conditions) {
		query += " WHERE "
...
func Query(queryString string) *[]common.LogMessage {
	...
	q := "SELECT rowid,timestamp,message FROM logs"
	...
	for rows.Next() {
		dbl := DbLogMessage{}
		err = rows.Scan(&dbl.RowId, &dbl.Timestamp, &dbl.Message)
		if err != nil {
			log.Println(err)
		}
		...
		if dbl.Timestamp.Valid {
			l.Timestamp = int(dbl.Timestamp.Int32)
		}

		if dbl.Message.Valid {
			l.Message = string(dbl.Message.String)
		}
